...
public class MongoDBProxy implements DBProxy {
	...
	public boolean hasConditionById(String id) {
		if(conditionCollection.find(Filters.eq("code", id)).first() == null){
			return false;
		}else {
			return true;
		}
	...
	public boolean hasDrugById(String id) {
		if(drugCollection.find(Filters.eq("code", id)).first() == null) {
			return false;
		}else {
			return true;
		}
	...
	public void save(Condition condition) throws IOException {
		...
		if(!this.hasConditionById(condition.getCode())) {
			...
			conditionCollection.insertOne(new Document()
					.append("code", condition.getCode())
					.append("name", condition.getName())
	...
	public void save(Drug drug) throws IOException {
		if(!this.hasDrugById(drug.getCode())) {
			drugCollection.insertOne(new Document()
					.append("code", drug.getCode())
					.append("name", drug.getName())
	...
	public void updateDrug(Drug updatedDrug) throws IOException {
		if(this.hasDrugById(updatedDrug.getCode())) {
			drugCollection.replaceOne(Filters.eq("code", updatedDrug.getCode()), new Document()
					.append("code",updatedDrug.getCode())
					.append("name", updatedDrug.getName())
	...
	public void updateCondition(Condition updatedCondition) throws IOException {
		...
		if(this.hasConditionById(updatedCondition.getCode())) {
			...
			conditionCollection.replaceOne(Filters.eq("code", updatedCondition.getCode()), new Document()
					.append("code", updatedCondition.getCode())
					.append("name", updatedCondition.getName())
					.append("dosageList", dosages));
		}else {
			throw new IOException("Database contains no Condition entry with given ID.");
		}
