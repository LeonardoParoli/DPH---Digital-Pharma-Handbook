...
public class ConditionTest {
	...
	public void instancedConditionMustBeConsistentAtStartTest() {
		...
		assertThat(condition.getName()).isNotNull().isInstanceOf(String.class).isNotEmpty().isNotBlank();
		assertThat(condition.getCode()).isNotNull().isInstanceOf(String.class).isNotEmpty().isNotBlank();
		assertThat(condition.getDosageList()).isNotNull().isInstanceOf(HashMap.class);
	...
	public void createdConditionCannotHaveDuplicateEntriesInDrugList() {
		assertThatThrownBy(() -> {
		  ...
		  .hasMessageContaining("Cannot create new Condition entry with duplicate entries in associated Drug list.");
	}
	...
	public void addDrugToConditionDrugListTest() {
		...
		assertThat(condition.addDrug(testDrug, 4)).isNotNull().isInstanceOf(Boolean.class).isTrue();
		assertThat(condition.getDosageList().values()).extracting(Dosage::getCode).anySatisfy(code ->assertThat(code).matches("004"));
	}
	...
	public void addDrugToConditionDrugListWithSameNameTest() {
		...
		assertThat(condition.addDrug(testDrug2,5)).isNotNull().isInstanceOf(Boolean.class).isTrue();
	}
	
	@Test
	public void ConditionHasSpecificDrugByCodeTest() {
		assertThat(condition.hasDrug("001")).isNotNull().isInstanceOf(Boolean.class).isTrue();
		assertThat(condition.hasDrug("004")).isNotNull().isInstanceOf(Boolean.class).isFalse();
	}
	...
	public void removeUnexistantDrugFromConditionDrugListTest() {
		assertThatThrownBy(() -> {
		  ...
		  .hasMessageContaining("Cannot remove unexisting drug entry.");
	}
	
	@Test
	public void removeExistantDrugFromConditionDrugListTest() {
		assertThat(condition.getDosageList().values()).extracting(Dosage::getCode).anySatisfy(code ->assertThat(code).matches("003"));
		assertThat(condition.removeDrug("003")).isNotNull().isInstanceOf(Dosage.class);
		assertThat(condition.getDosageList().values()).extracting(Dosage::getCode).noneSatisfy(code ->assertThat(code).matches("003"));
	}
	
	@Test
	public void conditionEqualsSuccessfulTest() {
		Condition otherCondition = new Condition(code,name,dosageList);
		assertThat(condition.equals(otherCondition)).isNotNull().isInstanceOf(Boolean.class).isTrue();
	}
